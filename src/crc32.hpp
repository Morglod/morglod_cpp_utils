...
// https://gist.github.com/wawiesel/ebda9309042dec57505aa33b4f4ac952
namespace crc32_detail {
    ...
    //runtime combine
    inline uint32_t combine_crc32_s(size_t idx, const char * str, uint32_t part) {
      return (part >> 8) ^ crc_table[(part ^ str[idx]) & 0x000000FF];
    }

    //runtime driver
    inline uint32_t crc32_s(size_t idx, const char * str) {
...
constexpr uint32_t uint_crc32(const char (&str)[len]) {
  return crc32_detail::crc32<len - 2>(str) ^ 0xFFFFFFFF;
}
...
inline uint32_t uint_crc32_s( const char* str ) {
  return crc32_detail::crc32_s(strlen(str)-1,str) ^ 0xFFFFFFFF;
}

//runtime that returns signed int
inline int int_crc32_s( const char* str) {
